...
function KatanaTerminal() {
  ...
  const sampleLogs = [
    { timestamp: '14:32:01', type: 'success', message: '✓ Helius RPC connected (42ms latency)' },
    { timestamp: '14:32:02', type: 'success', message: '✓ Jupiter API ready' },
    { timestamp: '14:32:03', type: 'success', message: '✓ Jito relay subscribed' },
    { timestamp: '14:32:04', type: 'success', message: '✓ WebSocket authenticated' },
    { timestamp: '14:32:05', type: 'info', message: '→ Listening for token launches...' },
    { timestamp: '14:33:12', type: 'detect', message: '🎯 NEW TOKEN: RAY | Liq: 45.2 SOL | MC: $2.3M' },
    { timestamp: '14:33:13', type: 'safe', message: '✓ Risk check passed (mint auth revoked)' },
    { timestamp: '14:33:14', type: 'success', message: '✓ BOUGHT 150 RAY @ 5.24 (slippage: 0.28%)' },
    { timestamp: '14:33:42', type: 'tp', message: '💰 TP1 HIT! Sold 45 RAY @ 10.48 (+99%)' },
    { timestamp: '14:34:00', type: 'info', message: '→ Position: 55 RAY | Entry: 5.24 | Current: 11.2' },
  ];
  ...
  const getLogColor = (type) => {
    switch (type) {
      ...
      case 'detect': return 'text-purple-400';
      case 'tp': return 'text-green-500 font-bold';
  ...
  const SystemLogs = () => (
    <div className="space-y-1 text-xs font-mono">
      {logs.map((log, idx) => (
        <div key={idx} className={`flex items-start space-x-2 ${getLogColor(log.type)}`}>
