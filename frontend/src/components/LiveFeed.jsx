...
function LiveFeed({ recentTrades = [] }) {
  const [liveFeed, setLiveFeed] = useState([
    ...
    },
    {
      id: 3,
      type: "alert",
      message: "High volatility detected on ORCA",
      level: "warning",
      timestamp: "14:35:35",
    },
    {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setLiveFeed((prev) => {
        ...
        const newItem = {
          id: Date.now(),
          type: ["price_update", "execution", "alert"][
          ...
          >
            <div className="flex justify-between items-start">
              <div className="flex-1">
                <div
                  className={`flex items-center space-x-2 ${getItemColor(item)}`}
                >
                  ...
                  {item.type === "alert" && <span>{item.message}</span>}
                </div>
              </div>
