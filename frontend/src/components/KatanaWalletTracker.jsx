    ...
    <div className="bg-gradient-to-br from-purple-950/40 to-black/60 border border-purple-500/30 rounded-xl overflow-hidden">
      ...
      <div className="space-y-2 p-3 max-h-64 overflow-y-auto">
        {wallets.length > 0 ? wallets.map((wallet, index) => (
          <div key={wallet.publicKey || index} className="p-3 bg-black/40 border border-purple-500/20 rounded-lg hover:border-purple-400/50 transition cursor-pointer group">
            <div className="flex items-start justify-between mb-2">
              <div>
                <div className="font-bold text-white text-sm">{wallet.name || 'Wallet ' + (index + 1)}</div>
                <div className="text-xs text-gray-500">{wallet.publicKey || wallet.address || 'Unknown'}</div>
              </div>
              <div className="text-right">
                <div className="font-bold text-white text-sm">{wallet.external ? 'External' : 'Internal'}</div>
                <div className="text-xs text-gray-500">{wallet.publicKey ? 'CONNECTED' : 'UNKNOWN'}</div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-3 gap-2 text-xs">
              <div className="p-2 bg-purple-500/10 rounded border border-purple-500/20">
                ...
                <div className="font-bold text-white">{wallet.external ? 'External' : 'On-chain'}</div>
                <div className="text-xs text-gray-400">{wallet.external ? 'Manual sign' : 'Auto trade'}</div>
              </div>
          ...
          <div className="p-4 text-sm text-gray-400">No wallets found yet. Connect a wallet to begin trading.</div>
        )}
      </div>
      ...
      <div className="px-3 pb-3">
        <button
          onClick={async () => {
            ...
            if (!publicKey) return;
            const name = window.prompt('Enter wallet name (optional):') || 'External Wallet';
