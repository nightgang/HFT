...
import { MessageCircle, ThumbsUp, ThumbsDown, TrendingUp, Zap } from 'lucide-react';
...
const SentimentAnalysis = () => {
  ...
  const fetchSentimentData = async () => {
    try {
      const response = await axios.get(`http://localhost:3001/api/sentiment-analysis?source=${filter}`);
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-gradient-to-br from-green-900/20 to-green-900/5 border border-green-500/30 rounded-lg p-6">
          ...
          <p className="text-xs text-gray-500 mt-2">Positive sentiment signals</p>
        </div>
        ...
        <div className="bg-gradient-to-br from-yellow-900/20 to-yellow-900/5 border border-yellow-500/30 rounded-lg p-6">
          ...
          <div className="text-3xl font-bold text-yellow-400">
            {sentiments.filter(s => s.sentimentScore <= 0.6 && s.sentimentScore >= 0.4).length}
          </div>
        ...
        <div className="bg-gradient-to-br from-red-900/20 to-red-900/5 border border-red-500/30 rounded-lg p-6">
          ...
          <p className="text-xs text-gray-500 mt-2">Negative sentiment signals</p>
        </div>
      </div>
      ...
      <div className="space-y-3">
        {sentiments.length === 0 ? (
          ...
          sentiments.map((item, idx) => (
            <div key={idx} className="bg-slate-900/50 border border-purple-500/20 rounded-lg p-4 hover:border-purple-500/40 transition-colors">
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-2">
                    <div className={`p-2 rounded ${getSentimentColor(item.sentimentScore)}`}>
                      {getSentimentIcon(item.sentimentScore)}
                    </div>
                    <div>
                      <h3 className="text-white font-semibold">{item.tokenMint}</h3>
                      <p className="text-sm text-gray-500">{item.source} • {new Date(item.createdAt).toLocaleString()}</p>
                    </div>
                  </div>
                  <p className="text-gray-400 text-sm">{item.content}</p>
                </div>
                <div className="text-right ml-4">
                  <div className={`text-2xl font-bold ${getSentimentColor(item.sentimentScore)}`}>
