...
const MONITORING = [
  ...
  { label: "Network Congestion", value: "32%",                Icon: Globe,    color: "#F59E0B", status: "Moderate" },
  { label: "Order Book Depth",   value: "$4.2M",             Icon: LineChart, color: "#8B5CF6", status: "Deep" },
  { label: "Pending Alerts",     value: "3",                 Icon: AlertCircle, color: "#EF4444", status: "High" },
];
...
const Dashboard = ({ dashboardConfig = {} }) => {
  ...
  const StatusBar = ({ realtime }) => {
    ...
    const botColor = statusColors[metrics.botStatus] || NEON.amber;
      ...
      >
        <div className="px-6 py-4 grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-7 gap-4 text-[11px] font-mono">
          ...
          <div className="flex items-center gap-3 p-3 bg-black/30 rounded-lg">
            <span className={`w-2.5 h-2.5 rounded-full ${metrics.botStatus !== "IDLE" ? "animate-pulse" : ""}`}
            ...
            <div>
              ...
              <div className="font-bold" style={{ color: botColor }}>{metrics.botStatus}</div>
            </div>
          </div>
          ...
          <div className="p-3 bg-black/30 rounded-lg">
            ...
            <div className="text-[#22D3EE] text-sm font-bold">{metrics.latency.toFixed(1)}ms</div>
          </div>

          {/* RPC Health */}
          <div className="p-3 bg-black/30 rounded-lg">
            <div className="text-gray-400 text-[10px] uppercase tracking-wider mb-1">RPC HEALTH</div>
            <div className={`px-2 py-1 rounded text-center text-xs font-semibold ${rpcColors[metrics.rpcHealth] || ""}`}>
          ...
          <div className="p-3 bg-black/30 rounded-lg sm:col-span-2 md:col-span-1">
            ...
            <div className="text-[#FBBF24] text-sm font-bold">{Math.floor(metrics.tps)}</div>
          </div>
  ...
  const MainPanel = () => {
    ...
    useEffect(() => {
      ...
      chart.addHistogramSeries({ color: "#8B5CF6", priceFormat: { type: "volume" } }).setData(buildVolume(candles));
    ...
    const handleRefresh = () => {
      ...
      chartRef.current.addHistogramSeries?.({ color: "#8B5CF6", priceFormat: { type: "volume" } }).setData(buildVolume(candles));
    };
  ...
  /* ══════════════════════════════════════════════════════════════════════
     11 · MONITORING CARD  (reusable)
     ══════════════════════════════════════════════════════════════════════ */
  const MonitorCard = ({ label, value, unit, Icon, accentColor, status }) => (
    <div className="monitor-card">
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <span className="monitor-label">{label}</span>
        <Icon size={15} style={{ color: accentColor, opacity: 0.65 }} />
      </div>
      <div style={{ display: "flex", alignItems: "baseline", gap: 6 }}>
        <span className="monitor-main" style={{ color: accentColor }}>{value}</span>
        {unit && <span className="monitor-sub">{unit}</span>}
      </div>
      <span className="pill" style={{ alignSelf: "flex-start", padding: "3px 10px", fontSize: "0.625rem", textTransform: "uppercase", letterSpacing: "0.06em", color: accentColor, background: `${accentColor}12`, border: `1px solid ${accentColor}28` }}>
        {status}
      </span>
    </div>
  );
  ...
  /* ══════════════════════════════════════════════════════════════════════
     13 · STATS CARD
     ══════════════════════════════════════════════════════════════════════ */
  const StatCard = ({ label, value, change, changeUp }) => {
    const isUp      = changeUp === true;
    const isNeutral = changeUp === null;
    return (
  ...
  /* ══════════════════════════════════════════════════════════════════════
     16 · INSIGHT ROW  (AI signals feed item)
     ══════════════════════════════════════════════════════════════════════ */
  const InsightRow = ({ label, detail, pulseColor }) => (
    <div className="feed-row">
      ...
      <span className="live-dot" style={{ background: pulseColor || "#94a3b8" }} />
    </div>
  );
  ...
  /* ══════════════════════════════════════════════════════════════════════
     ...
     ══════════════════════════════════════════════════════════════════════ */
  const DesktopView = () => {
    const latency   = systemStatus?.latency ?? "12";
    return (
  ...
  /* ══════════════════════════════════════════════════════════════════════
     24 · STATUS BAR (render as first cell in desktop grid)
     ══════════════════════════════════════════════════════════════════════ */
