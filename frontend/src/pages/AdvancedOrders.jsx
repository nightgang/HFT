...
import { ArrowUpRight, ArrowDownLeft, Clock, AlertCircle, Trash2, Plus } from 'lucide-react';
...
const AdvancedOrders = () => {
  ...
  const fetchOrders = async () => {
    try {
      const response = await axios.get('http://localhost:3001/api/advanced-orders');
  ...
  const handleDeleteOrder = async (orderId) => {
    try {
      await axios.delete(`http://localhost:3001/api/advanced-orders/${orderId}`);
      fetchOrders();
    } catch (error) {
  ...
  const getOrderTypeIcon = (type) => {
    return type === 'stop_loss' ? <ArrowDownLeft className="w-4 h-4" /> : <ArrowUpRight className="w-4 h-4" />;
  };
    ...
    <div className="space-y-6">
      ...
      {showModal && (
        <div className="bg-slate-900/80 border border-purple-500/30 rounded-lg p-6 space-y-4">
          <h2 className="text-xl font-bold text-white">Create Advanced Order</h2>
      ...
      <div className="space-y-3">
        {orders.length === 0 ? (
          <div className="text-center py-12 text-gray-500">No advanced orders yet</div>
        ) : (
          orders.map((order) => (
            <div key={order.id} className="bg-slate-900/50 border border-purple-500/20 rounded-lg p-4 flex items-center justify-between hover:border-purple-500/40 transition-colors">
              <div className="flex items-center gap-4 flex-1">
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2">
                    <h3 className="text-white font-semibold capitalize">{order.orderType}</h3>
              ...
              <div className="text-right space-y-1">
                <div className="text-white font-semibold">{order.amount} tokens</div>
